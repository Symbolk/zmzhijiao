...
<!--{if empty($post['deleted'])}-->
    <!--{if $post['first']}--> 
        <table id="pid$post[pid]" class="plhin viewthread_table bus_viewthread_br" summary="pid$post[pid]" cellspacing="0" cellpadding="0">
<tr>
	...
	<td class="plc"<!--{if $close_leftinfo}--> style="width:100%"<!--{/if}-->>
		<div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:48px"<!--{/if}-->>
			<div class="bus_fl">
					<span class="autorpic"><a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" title="$_G[forum_thread][author]">{echo avatar($post['authorid'], 'small')}</a></span>
				<div class="buszb">
					<span>
					...
					<span>{lang reply}:$_G[forum_thread][allreplies]</span>
					<span class="pipe show">|</span>
					<span>{lang show}:$_G[forum_thread][views]</span>
					<span class="pipe show">|</span>
					...
					<span class="xg1">
						...
						<!--{/if}-->
						<span class="pipe show">|</span><a href="forum.php?mod=viewthread&tid=$_G[tid]$fromuid" onclick="return copyThreadUrl(this, '$_G[setting][bbname]')" {if $fromuid}title="{lang share_url_copy_comment}"{/if}>{lang share_url_copy}</a>
					</span>
				</div>
			</div>
				...
				<!--{if !$postcount && !$_G['forum_thread']['archiveid'] && $post['first'] }-->
					<div id="fj" class="y">
                    	<span class="y bus_jp" style=" width:24px;">
                        	<a href="home.php?mod=spacecp&ac=favorite&type=thread&id=$_G[tid]" id="k_favorite" onclick="showWindow(this.id, this.href, 'get', 0);" onmouseover="this.title = $('favoritenumber').innerHTML + ' {lang activity_member_unit}{lang thread_favorite}'" title="{lang fav_thread}"><img src="$_G['style'][styleimgdir]/pl_t_fav.gif" alt="{lang thread_favorite}" />	</a>
                        </span>
                        <span class="y bus_jp" style=" width:26px;">
                        	<input type="text" class="p_fre z" size="2" onkeyup="$('fj_btn').href='forum.php?mod=redirect&ptid=$_G[tid]&authorid=$_GET[authorid]&postno='+this.value" onkeydown="if(event.keyCode==13) {window.location=$('fj_btn').href;return false;}" title="{lang thread_redirect_postno_tips}" />
						<a href="javascript:;" id="fj_btn" class="z" title="{lang thread_redirect_postno_tips}"></a>
                        </span>
						<div class="clear"></div>
					</div>
				<!--{/if}-->
	...
	<td class="plc"<!--{if $close_leftinfo}--> style="width:100%"<!--{/if}-->>
		<div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:48px"<!--{/if}-->>
			...
			<div class="pti" style=" display:inline-block;">
				...
				<!--{/if}-->
        	<!--{if $_G['setting']['authoronleft']}-->
                    <div class="authi" style=" display:inline-block;"><a href="home.php?mod=space&uid=$post['authorid']" target="_blank" {if $post[groupcolor]} style="color: $post[groupcolor]"{/if}>$post[author]</a>$authorverifys <span style="margin-right: 5px;"><a href="http://wangxiao.xiniuzb.com/org/5883/course" target="_blank">{$post[groupicon]}</a></span><span><a href="home.php?mod=spacecp&ac=usergroup&gid=$post['authorid']" target="_blank">{$post[authortitle]}</a></span></div>
            <!--{/if}-->
